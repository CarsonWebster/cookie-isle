...
async function handleCreateSession(request, env, stripe, origin) {
  try {
    ...
    if (gift_box && gift_box.price_id) {
      lineItems.push({
        price: gift_box.price_id,
        quantity: 1,
      });
    }

    // Add tip as ad-hoc price (no pre-existing price_id needed)
    if (tip && tip > 0) {
    ...
    const metadata = {
      ...
      pickup_time: fulfillment.slot
        ? `${fulfillment.slot.date} ${fulfillment.slot.startTime}`
        : "N/A",
      delivery_address:
