<script lang="ts">
	...
	// Check if user is authenticated (from layout server load)
	let isAuthenticated = $derived(data.authenticated === true);
...
{:else}
	<!-- Unauthenticated: render page content without sidebar (login page) -->
	{@render children()}
{/if}
